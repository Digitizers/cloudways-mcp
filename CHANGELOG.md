...
- **`app_get` and `server_get` leave every step that only needed metadata** (Medium — "credential-
  ...
  roster. Whether a CDN or proxy sits in front is decided by DNS — A and AAAA both, address lines only, asked of a **public resolver** (`dig @1.1.1.1`) so split-horizon DNS on a VPN cannot hide a CDN behind a local answer, filtered with `awk` rather than `grep` — and guarded so that an empty answer (resolver blocked or erroring: `dig` exits 9, the pipeline exits 0 and prints nothing, measured) fails the gate instead of vacuously passing it, so a name with no AAAA record does not
