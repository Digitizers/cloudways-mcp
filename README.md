...
**Zero-config connection via env var.** The repo commits a placeholder-only [`.mcp.json`](.mcp.json) that reads the token from the `CLOUDWAYS_ACCESS_TOKEN` environment variable — set it in your shell (devices) or in the claude.ai cloud environment's env vars (web/phone sessions), and the `cloudways` connection comes up on its own; leave it unset and the server simply doesn't start. Real tokens never go into the file — it is tracked in git. **Migrating from a local gitignored `.mcp.json`:** move your token to the env var (or `claude mcp add -s user`), delete the local file, then pull.
